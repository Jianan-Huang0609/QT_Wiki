...
- 在 TODO 和 Spec 中补齐 `External Architecture References` 小节，按固定落点记录 Self-RAG、VMAO、GraphRAG、DSPy、Enterprise QA lessons 和 MEQA，明确只吸收 thin guardrail、bounded workflow、later graph summaries、eval-driven optimization、evidence boundary/human review、tool-specific agents 等工程化映射。
...
- 完成 `RAG-03 表格检索诊断 + route-gated semantic retriever`：`semantic-rag-smoke` 新增 `route_gated_semantic` backend，role/reference route 继续走 lexical/rule baseline；真实 Azure embedding smoke 中 route-gated recall@8 回到 `1.0`，citation drift 从 `2` 降到 `0`。新增 `Tool.evals.table_retrieval_diagnostic` 和 `Design/review-artifacts/table-retrieval-diagnostic.md`，真实 MI PEP 表格问题已命中 `table_lookup`，但 scoped table chunk 为 `0`，失败分类为 `parser_provider_gap`。结论：上下文/Router/Planner 已能到表格意图层，当前还没到业务 table/cell anchor level；下一步需补 table-aware chunk metadata 或触发式 parser provider。
- 补充 Router/RAG 下一步落地边界：`_session_intent_route_shadow()` 标记硬编码 confidence 的退役条件，需等 `CHAT-05C` route evolution eval 能稳定校准 `route_match/evidence_likely`；`RAG-03` 调整为表格检索诊断 + route-gated semantic retriever，semantic 先只在非 role/reference route 启用；新增 `CHAT-05C` backlog，专门覆盖“容易遗漏 / 准备事项 / 注意事项”类 fallback mismatch 问题。
