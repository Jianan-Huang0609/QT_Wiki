...
- Ask Workspace v1 已能跑 selected-doc query、Answer Run、Reference Card、Note pin，但仍承载了过多历史状态。
- Source、Tree、Graph、Review Queue、Quality、JSON、Admin、Chat 曾经混在一个界面里，用户不知道下一步做什么。
- Reference 卡已经比 `c1/c2` 好，但还需要和 block/table/cell anchors 进一步结合。
...
```text
Left: File-level source scope list
Center: Chat transcript + Answer Run + composer
Right: Reference Viewer + Session Note
```
...
- 浏览器 smoke：上传/选择 PEP -> 自动解析状态 -> Ask Workspace 连续两问 -> pin note；低置信项可进入 Review Gate。
- Reference Card 显示文件、页码/章节、quote。
- Answer Run 六步来自后端。
